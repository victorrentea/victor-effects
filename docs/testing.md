...
- `GET /test/coffee/pop` — burst ONE ☕ mid-screen right now, dissolve and webhook included, without a salvo and without a hand on the pot. NB **not read-only** in the wider sense: the webhook really fires, and whatever is listening for it really acts. Point `eventWebhook` at an `nc -l` to watch the GET arrive without triggering the real payoff
