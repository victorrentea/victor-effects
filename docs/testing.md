...
- `GET /test/microwave` — fire the ⏲️ Microwave overlay now, **with its sound** (~4.4 s: 2.7 s of ticking at a closed door, then the BING and the door swinging open); same as `/effect/microwave`. Unlike the other effect hooks this one is not silent — the sync between the bell and the door is the whole thing being tested. No tablet tile drives it since 2026-09-25 (#61 is now Bocelli's Hallelujah, with no effect)
...
- `GET /test/coffee` — spawn 3 ☕ rising straight up from the bottom-left (the **chimney** flight — swaying gently, fading near the top; the mouse never steers them). Move the cursor onto one: the pointer becomes a 🫖 that bends toward the cup, pouring a stream of drops from its spout tip that ends at the coffee's surface, and the cup **stops where it was caught and grows** (to 1.7× over 1.2 s, with a warm glow); full, it **pops** — a small local pixel dissolve — and is gone. Three at once is not a salvo, so nothing shakes and no other cup is cleared. Each pop fires one `?type=coffee-popped&x=&y=` at `eventWebhook`, after the explosion; with no webhook configured the ☕ still fills and pops
- `GET /test/coffee/pop` — pop ONE ☕ mid-screen right now, dissolve and webhook included, sized as a real payoff would be at this moment (`CoffeeBurst`: small with ≤3 cups on screen, bigger toward 10, the big burst — which also fades out every other ☕ — while a salvo is armed), without a hand on the pot. NB **not read-only** in the wider sense: the webhook really fires, and whatever is listening for it really acts. Point `eventWebhook` at an `nc -l` to watch the GET arrive without triggering the real payoff
- `GET /test/coffee/storm` — a salvo of 8 ☕ inside one second, past the **more than 3 in a second** threshold that ARMS explosions. The cups rise exactly like lone ones; from now on a cup the pot touches does not fill — it grows to 3× while shaking harder and harder and **bursts where it is**, on its path, into 22×22 fragments thrown across the screen. Disarms on its own once no cup is left on screen or 10 s after the salvo; `/effect/stop-all` resets it at once
...
- `CoffeePourRenderTests` — the ☕ pour RENDERED offscreen through `CARenderer` (emitters and masks drawn as the window server draws them, no display touched): the frame is drawn twice at one instant, with and without the stream, and any stream pixel below the cup's coffee surface fails it. Leaves `coffee-pour.png` in `$TMPDIR`. Two harness lessons are in its code: give `CARenderer` the read-back's own command queue (on a second queue the blit overtakes the frame and the bytes are half the previous frame), and give the root `actions = ["sublayers": NSNull()]` like the overlay's NSView-backed root (a bare `CALayer` fades its whole tree on every sublayer added, ghosting anything that moves). `CoffeeSurfaceTests` holds the pure halves: the measured surface, `fallTime`, and `CoffeeBurst`'s thresholds
