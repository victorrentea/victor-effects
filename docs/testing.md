...
- `GET /test/red-button` — grow the 🔴 big red button out of the pointer now (silent); same as `/effect/red-button`. **Interactive, and the only effect that outlives its clip**: it stays until it is clicked, until Esc, until `/effect/stop-all`, or until its own 20 s deadline, then shrinks back into the pixel it grew out of. `GET /test/red-button/stop` takes it away early. The tablet drives it via `GET /sound/pressed/07_animated_phone.mp3` (mapped to `red-button` in `SoundEffectMap`, deliberately with **no** `onStop` entry) while the routed `/sound/play/07_animated_phone.mp3` plays the clip. `GET /state` reports it as `red-button` in `activeEffects` while it is up — which is the headless way to see it, since the artwork is on the built-in Retina and the *press* needs a real mouse. Pressing it logs `🔴 red button clicked — origin P=(x,y)` and then `🔴 red button: click passed through to P=(x,y)` — `onButtonClicked` is wired to `RedButtonController.deliverClickBelow(at:)`, which posts a real left click at P once the hit panel has gone deaf, so the press reaches whatever the button was covering. NB it needs `red_button.gif` in `assetsDir`; without it the route answers 200 and logs `showRedButton: no red_button.gif in …`
...
- `PeekWhipJumpTests` — the ⌘⌃Q mascot's scramble on a crack: the whole route stays on the screen (top bezel *and* the tight left one), the 15% apex and the leftward hop both fit without the clamp doing the work, he takes off four times and still lands on the pixel he left, `keyTimes` lines up with the route, both cut-outs run it identically, and — by parsing `WhipOverlay.swift` — every crack still goes through the `cracked()` funnel rather than calling `playCrack()` directly
